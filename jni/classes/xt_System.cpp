...
static jstring xtSystem_getDeviceInfo(JNIEnv *env, jclass clazz)
{
    ...
    const char *override_info = getenv("ICERAGE_DEVICE_INFO");
    if (override_info) {
 ...
 * a scale of 1 means. Nothing inside libicerage.so calls
...
static jstring xtSystem_getInputDeviceNameByDeviceId(JNIEnv *env, jclass clazz, jint device_id)
{
     ...
     *
     * ICERAGE_PAD_NAME overrides it, so the other profiles can be tried
     * without a rebuild.
     */
    const char *forced = getenv("ICERAGE_PAD_NAME");
    if (forced && *forced)
