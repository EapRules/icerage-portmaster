...
cp tools/grab_screen.sh "$DST/icerage/grab_screen.sh"
chmod +x "$DST/icerage/grab_screen.sh"

# Same launcher, plus a background capture loop and its own log.
sed -e 's|export PORT_32BIT="Y"|export PORT_32BIT="Y"\n\n# Throwaway entry: screenshots itself, see tools/make_capture_entry.sh\n(\n  for delay in 20 15 15 20; do\n    sleep "$delay"\n    "/$directory/ports/icerage/grab_screen.sh" "/$directory/ports/icerage/shot-$(date +%s)" >/dev/null 2>\&1\n  done\n) \&|' \
    -e 's|\$GAMEDIR/log\.txt|$GAMEDIR/log-capture.txt|g' \
    "ports/Ice Rage.sh" > "$DST/Ice Rage (captura).sh"
chmod +x "$DST/Ice Rage (captura).sh"

echo "wrote:"
echo "  $DST/Ice Rage (captura).sh"
echo "  $DST/icerage/grab_screen.sh"
echo
...
echo "ports/icerage/ at roughly 20s, 35s, 50s and 70s."
