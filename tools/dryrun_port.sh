...
# run: it builds a throwaway PortMaster installation around ports/Ice Rage.sh
...
#   * "/$directory/ports/icerage" resolves to where the port was installed
#   * cd, the log.txt redirect and mkdir of ICERAGE_DATA_DIR all succeed
...
#       icerage-build tools/dryrun_port.sh /apk/icerage_play.apk
set -uo pipefail

APK="${1:?usage: dryrun_port.sh <path to icerage apk>}"
ROOT=/tmp/dryrun
PM="$ROOT/home/.local/share/PortMaster"
GAMEDIR="/roms/ports/icerage"
...
cp "build/icerage"                  "$GAMEDIR/icerage"
cp "ports/icerage/icerage.gptk"   "$GAMEDIR/icerage.gptk"
cp "ports/Ice Rage.sh"              "/roms/ports/Ice Rage.sh"
...
MISSING=""
covq=$(needed_of "$GAMEDIR/icerage"); covseen=" "
while [ -n "${covq// /}" ]; do
    covnext=""
    for s in $covq; do
...
ln -sf "$APK" "$GAMEDIR/icerage.apk"
chmod -x "$GAMEDIR/icerage"   # prove the launcher's chmod +x is doing work
...
export ICERAGE_FRAME_LIMIT="${ICERAGE_FRAME_LIMIT:-40}"

echo "[dryrun] executing the launcher"
bash "/roms/ports/Ice Rage.sh"
RC=$?
...
check "ICERAGE_DATA_DIR created"                test -d "$GAMEDIR/gamedata"
check "binary made executable"                   test -x "$GAMEDIR/icerage"
