 ...
 * libicerage.so out of it with the bionic ELF loader vendored from
 * gmloader-next.
 *
...
/* The game ships one ABI only; there is no arm64 build of Ice Rage.
...
static const char *kNativeLib     = "libicerage.so";
static const char *kNativeLibPath = "lib/armeabi-v7a/libicerage.so";
 ...
 *
 * ICERAGE_RENDER selects one; it exists because only a device with a
...
{
    const char *v = getenv("ICERAGE_RENDER");
 ...
 * KMSDRM that is the panel. ICERAGE_PANEL_W/H overrides it - both for a CFW
...
static void resolve_panel(int *out_w, int *out_h)
{
    ...
    const char *env_w = getenv("ICERAGE_PANEL_W");
    const char *env_h = getenv("ICERAGE_PANEL_H");
    if (env_w && *env_w && env_h && *env_h) {
        w = atoi(env_w);
        h = atoi(env_h);
        trace("panel: overridden to %dx%d by ICERAGE_PANEL_W/H", w, h);
    }
 ...
 *
 * The PortMaster launcher points ICERAGE_DATA_DIR at the port's own data
...
{
    const char *base = getenv("ICERAGE_DATA_DIR");
    if (!base || !*base)
        base = "gamedata";
...
int main(int argc, char **argv)
{
     ...
     * verify it on, so ICERAGE_NO_FORCE_GLES exists to turn it back off.
     */
    if (!getenv("ICERAGE_NO_FORCE_GLES")) {
    ...
    SDL_Window *window = SDL_CreateWindow(
        "Ice Rage", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
    ...
    /*
     * The shipped splash override is a 4:3 crop of the game's own widescreen
     ...
     *
     * ICERAGE_ASSET_OVERRIDE_FORCE keeps it on regardless, for someone who put
     * their own artwork there and means it.
     */
    if (getenv("ICERAGE_ASSET_OVERRIDE") &&
        !getenv("ICERAGE_ASSET_OVERRIDE_FORCE")) {
        float aspect = (float)draw_w / (float)draw_h;
        if (aspect > 1.40f) {
            unsetenv("ICERAGE_ASSET_OVERRIDE");
            trace("asset overrides disabled: the %dx%d panel is not 4:3, so the "
                  "APK's own artwork fits it better (ICERAGE_ASSET_OVERRIDE_"
     ...
     * APK, so libicerage.so is the only object that gets mapped.
     */
    ...
    if (!data_path) {
        fatal("cannot create the save directory '%s': %s\n"
              "       Set ICERAGE_DATA_DIR to a writable location.",
     ...
     */
    const char *gl_wait_env = getenv("ICERAGE_GL_TIMEOUT");
    ...
    if (!gl_ready) {
        fatal("the game never brought GL up within %ds: EGL context %s,\n"
              "       %d shader(s) compiled. Set ICERAGE_GL_TIMEOUT to wait\n"
    ...
    /*
     * A bounded run. ICERAGE_FRAME_LIMIT stops the process after that many
     ...
     */
    const char *limit_env   = getenv("ICERAGE_FRAME_LIMIT");
     ...
     */
    const char *stall_env = getenv("ICERAGE_FRAME_STALL");
