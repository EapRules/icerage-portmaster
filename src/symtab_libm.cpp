 ...
 */
extern "C" int icerage_open(const char *path, int flags, ...)
{
    mode_t mode = 0;
...
extern "C" int icerage_ioctl(int fd, int request, ...)
{
    va_list ap;
    va_start(ap, request);
 ...
 * Ice Rage's does, so leaving it unresolved fails the load outright.
 *
 ...
 */
extern "C" void icerage_assert2(const char *file, int line,
...
DynLibFunction symtable_libm[] = {
    ...
    NO_THUNK("open",  (uintptr_t)&icerage_open),
    NO_THUNK("ioctl", (uintptr_t)&icerage_ioctl),
    /* No float crosses the boundary, so no ABI bridge is needed. */
    NO_THUNK("__assert2", (uintptr_t)&icerage_assert2),
