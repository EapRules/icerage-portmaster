...
static void face_layout_init(void)
{
    const char *layout = getenv("ICERAGE_FACE_LAYOUT");
...
{
    /* ICERAGE_CURSOR_TEST parks the cursor on screen with no pad attached,
    ...
    static int forced = -1;
    if (forced < 0)
        forced = getenv("ICERAGE_CURSOR_TEST") ? 1 : 0;
 ...
 * hardware attached. ICERAGE_SYNTH_INPUT=<ms> taps the screen centre at that
...
static void synth_input_tick(void)
{
    ...
    if (interval < 0) {
        const char *env = getenv("ICERAGE_SYNTH_INPUT");
...
extern "C" int ALooper_pollAll(int timeoutMillis, int *outFd, int *outEvents, void **outData)
{
    ...
    {
        static long calls = 0;
        static bool probe = getenv("ICERAGE_POLL_PROBE") != NULL;
