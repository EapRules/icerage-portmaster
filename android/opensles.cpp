 ...
 *
 * Ice Rage does all of its audio through OpenSL ES: it creates an engine, an
...
static ABI_ATTR SLresult player_Realize(SLObjectItf self, SLboolean async)
{
    ...
    if (g_audio_up && p->decodes) {
         ...
         *
         * ICERAGE_AUDIODEV overrides the whole search with one name.
         */
        const char *forced = getenv("ICERAGE_AUDIODEV");

        if (forced && *forced) {
            p->device = SDL_OpenAudioDevice(forced, 0, &want, &have,
                                            SDL_AUDIO_ALLOW_FREQUENCY_CHANGE);
            if (!p->device)
                warning("OpenSL ES: ICERAGE_AUDIODEV=%s failed: %s\n",
                        forced, SDL_GetError());
        } else {
            ...
            if (!p->device) {
                 ...
                 * ICERAGE_NO_PREFER_SPEAKER restores the old behaviour if the
                 * heuristic ever guesses wrong somewhere else.
                 */
                const char *first_err = SDL_GetError();
                int n = SDL_GetNumAudioDevices(0);
                bool prefer_speaker = (getenv("ICERAGE_NO_PREFER_SPEAKER") == NULL);
...
/*
 * The table the loader binds into libicerage.so for its libOpenSLES.so
 * imports.
 */
#include "so_util.h"
#include "thunk_gen.h"
