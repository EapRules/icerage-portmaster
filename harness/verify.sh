#!/usr/bin/env bash
#
# Autonomous verification harness for the Ice Rage port.
#
...
IMAGE="icerage-build"
APK="${ICERAGE_APK:-$PORT_DIR/../icerage_play.apk}"
TIMEOUT="${TIMEOUT:-90}"
...
#   2 load       libicerage.so loads with every symbol resolved
...
if docker run --rm -v "$PORT_DIR":/src -w /src "$IMAGE" make -j"$(nproc 2>/dev/null || echo 4)" \
    ...
    if docker run --rm -v "$PORT_DIR":/src -w /src "$IMAGE" \
         file build/icerage 2>/dev/null | grep -q "ELF 32-bit.*ARM"; then
        REACHED=1
        log "M1 ok"
    else
...
if [ ! -f "$APK" ]; then
    fail "APK not found at $APK (set ICERAGE_APK)"
    exit $REACHED
fi
...
# letterboxed everywhere else. ICERAGE_PANEL_W/H force a panel size, and
# ICERAGE_RENDER picks how the game is fitted to it; both are forwarded only
# when set, so a plain run is unchanged.
# ICERAGE_FRAME_STALL travels with them: a large panel under llvmpipe inside
...
PANEL_ENV=()
for var in ICERAGE_PANEL_W ICERAGE_PANEL_H ICERAGE_RENDER ICERAGE_SCALE \
           ICERAGE_FRAME_STALL; do
...
docker run --rm \
    ...
    -e LOADER_TRACE=1 \
    -e ICERAGE_FRAME_LIMIT="$((MIN_FRAMES * 4))" \
    "$IMAGE" \
    timeout --signal=INT "$TIMEOUT" \
    qemu-arm -L /usr/arm-linux-gnueabihf \
        ./build/icerage "/apk/$(basename "$APK")" \
